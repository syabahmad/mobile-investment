        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
          <div className="space-y-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              <h1 className="text-xl sm:text-2xl font-bold text-slate-900">Systems</h1>
            ...
            <p className="text-xs sm:text-sm text-slate-500">Create and manage investment categories</p>
          </div>
          <button onClick={openCreate}
            className="btn btn-primary text-xs gap-2 self-start sm:self-auto">
          ...
          {filtered.length === 0 ? (
            ...
            <>
              ...
              <div className="space-y-2 p-3 sm:hidden">
                {filtered.map((cat) => (
                  <div key={cat._id} className={`rounded-xl bg-slate-50 p-3 border border-slate-100 ${!cat.isActive ? 'opacity-60' : ''}`}>
